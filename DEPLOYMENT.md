...
| Images broken on the storefront but load fine in the admin dashboard | Confirms it's specifically the `next/image` domain allow-list, not a bad/missing image URL — the admin just renders a plain `<img>`, unaffected by `remotePatterns` | Check which commit your **current production** Vercel deployment actually built from (Deployments tab, not just the domain) — if it predates the `images.remotePatterns` fix in `next.config.js`, it never picked it up. Push any new commit to `main` (or manually redeploy from the latest commit) to force a rebuild |
